...
fs.readFile('./factura.xml', function (err, data) {
  ...
  const fecha = json['cfdi:Comprobante']['Fecha'].replace('T', ' ');
  ...
  const descuento = json['cfdi:Comprobante']['Descuento']
    ? json['cfdi:Comprobante']['Descuento']
    : 0;
  ...
  fs.writeFile(
    `./queries/${dir}/insert-factura.sql`,
    `
    INSERT INTO Factura(
  ...
  conceptos.forEach((concepto) => {
    ...
    const descuentoPorcentaje =
      concepto['Descuento'] > 0
        ? parseFloat(concepto.Importe) / parseFloat(concepto['Descuento'])
        : 0;

    console.log(descuentoPorcentaje);

    fs.writeFile(
      `./queries/${dir}/insert-detalle-factura-${concepto.NoIdentificacion}.sql`,
      `
      INSERT INTO DetalleFactura (
        ...
        idFactura,idCatProducto,'${serie}',
        '${folio}','${rfc}','${uuid}',
        '${concepto['NoIdentificacion']}','${concepto['Unidad']}','${concepto['Descripcion']}',
        ${concepto['Importe']},${concepto['Importe']},${concepto['Descuento']},
        ${descuentoPorcentaje},'${fecha}',58703,
        ${concepto['ClaveProdServ']},'${concepto['ClaveUnidad']}',${concepto['Cantidad']}
      )
      `,
      function (err) {
        if (err) throw err;
    ...
    fs.writeFile(
      `./queries/${dir}/insert-factura-detalle-impuesto-${concepto.NoIdentificacion} .sql`,
      `
      INSERT INTO DetalleFacturaImpuesto(
        ...
        ${concepto['cfdi:Impuestos']['cfdi:Traslados']['cfdi:Traslado']['Importe']},'${fecha}',58703,
        2,0.16)
      `,
    ...
    fs.writeFile(
      `./queries/${dir}/insert-cargo-${concepto.NoIdentificacion}.sql`,
      `
      INSERT INTO Cargo(
